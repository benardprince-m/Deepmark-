...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  const { data: integration } = await supabaseAdmin.from('integrations').select('id').eq('id', id).eq('workspaces.user_id', userPayload.userId).is('deleted_at', null).single();
  if (!integration) return notFoundResponse('Integration not found');
  const now = new Date().toISOString();
  await supabaseAdmin.from('integrations').update({ status: 'revoked', deleted_at: now, updated_at: now }).eq('id', id);
